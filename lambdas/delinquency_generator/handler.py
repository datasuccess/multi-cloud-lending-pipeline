"""Lambda entry point for the `delinquencies` generator.

Two parents:
- `loan_drawdowns` (today's): FK universe + drawn_amount basis.
- `payments` (all partitions through `ingest_date`): cumulative
  scheduled-vs-actual gap per drawdown.

Event payload (all optional):
  - ingest_date: "YYYY-MM-DD" default today UTC

No `seed` knob — this generator is deterministic.
"""
...
from lambdas.shared.parent_partition import (
    all_success_partitions_through,
...
# Late portfolio is a small subset of drawdowns. Default floor is intentionally
# low; tests override via env. Spec target: ~1500 rows/day in prod.
MIN_ROWS = int(os.environ.get("MIN_ROWS", 100))
DRAWDOWNS_SOURCE = "loan_drawdowns"
PAYMENTS_SOURCE = "payments"

DRAWDOWN_COLUMNS = ["drawdown_id", "customer_id", "drawn_amount"]
PAYMENT_COLUMNS = [
    "drawdown_id",
    "scheduled_amount",
    "actual_amount",
...
logger = get_logger("delinquencies-generator")
metrics = get_metrics("delinquencies-generator")
...
class RunResult:
    ...
    payments_partition_uris: list[str]
...
def _load_payments_history(
    base_uri: str, *, ingest_date: date
) -> tuple[dict[str, list[dict]], list[str]]:
    """All payments ≤ ingest_date, grouped by drawdown_id."""
    partitions = all_success_partitions_through(
        base_uri, PAYMENTS_SOURCE, through=ingest_date
    )
    by_drawdown: dict[str, list[dict]] = defaultdict(list)
    for partition_uri in partitions:
        cols = read_parent_columns(partition_uri, PAYMENT_COLUMNS)
        n = len(cols["drawdown_id"])
        for i in range(n):
            by_drawdown[cols["drawdown_id"][i]].append(
                {c: cols[c][i] for c in PAYMENT_COLUMNS}
            )
    return dict(by_drawdown), partitions
...
def run(
    *,
    ...
    payments_by_drawdown, payments_uris = _load_payments_history(
        base_uri, ingest_date=ingest_date
    )
    if not payments_uris:
        raise ParentNotFound(
            f"No `_SUCCESS`'d {PAYMENTS_SOURCE} partition through {ingest_date}"
        )
    logger.append_keys(payments_partitions=len(payments_uris))
    ...
    # No expected_rows — delinquencies count is derived; gate on min_rows only.
    errors = validate_table(
        table_back,
        expected_rows=table_back.num_rows,
        schema=DELINQUENCIES_SCHEMA,
        min_rows=MIN_ROWS,
    )
    ...
    bucket_counts = Counter(r["dpd_bucket"] for r in rows)
    ...
    result = RunResult(
        ...
        payments_partition_uris=payments_uris,
    ...
    record_run_metrics(
        ...
        channel_counts=dict(bucket_counts),
    )
...
class ParentNotFound(Exception):
    """Drawdowns or payments parent partition missing."""
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:
    ...
    logger.info("delinquencies_generator.start", extra={"trigger": trigger})
    ...
    try:
        ...
        logger.warning("delinquencies_generator.skipped", extra={"run_id": exc.run_id})
        return {"status": "skipped", "run_id": exc.run_id}

    logger.info(
        "delinquencies_generator.success",
        extra={
